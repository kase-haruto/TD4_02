...
void PunchAttack::ShowGui() {
	param_.ShowGui();
}

CalyxEngine::SerializableObject& PunchAttack::SerializableParam() {
	return param_;
}
...
void PunchAttack::UpdateAttack(BaseEnemy& self, float dt) {
	attackTimer_ += dt;
...
void PunchAttack::EndAttack(BaseEnemy& self) {
	isAttacking_ = false;
	attackTimer_ = 0.0f;
	RemoveAttackHitbox();
...
void PunchAttack::CreateAttackHitbox(BaseEnemy& self) {
	if (attackHitbox_) {
		return;
	}
...
void PunchAttack::UpdateAttackHitbox(BaseEnemy& self) {
	if (!attackHitbox_) {
		return;
	}
...
void PunchAttack::RemoveAttackHitbox() {
	if (!attackHitbox_) {
		return;
	}
